...
async function getRecentEvents() {
  try {
    const result = await db.execute({
      sql: `SELECT al.*, a.name as api_name, a.category
            FROM alerts al
    ...
    async start(controller) {
      const send = async () => {
        try {
          ...
          controller.enqueue(encoder.encode(`data: ${JSON.stringify({ apis, stats, events })}\n\n`));
        } catch (e) {
...
export { getLiveApis, getGlobalStats, getRecentEvents };
