...
export default function MarketplacePage() {
  ...
  const filteredApis = useMemo(() => {
    ...
    switch (sort) {
      case 'uptime':
        ...
        break;
      case 'fastest':
        result.sort((a, b) => (a.responseTime || Infinity) - (b.responseTime || Infinity));
        break;
      case 'risk':
        result.sort((a, b) => (b.riskScore || 0) - (a.riskScore || 0));
        break;
      case 'watched':
        // Default sort since we don't have watcher count from API
        result.sort((a, b) => (watching.has(b.id) ? 1 : 0) - (watching.has(a.id) ? 1 : 0));
        break;
    }
  ...
  const top5Reliable = useMemo(() => {
    return [...apis].sort((a, b) => (b.uptime || 0) - (a.uptime || 0)).slice(0, 5);
  }, [apis]);
  ...
  const generateSparkline = (apiId: string) => {
    // Generate deterministic pseudo-random heights based on API id
    const seed = apiId.split('').reduce((acc, c) => acc + c.charCodeAt(0), 0);
    return Array.from({ length: 20 }, (_, i) => {
      const hash = Math.sin(seed * (i + 1) * 0.001) * 10000;
      return Math.abs(Math.floor((hash % 100)));
    });
  };
      ...
      >
            ...
            {navLinks.map((link) => (
              <Link
                ...
                style={{
                  ...
                  background: link.active ? 'var(--accent-glow)' : 'transparent',
                }}
              >
                {link.label}
              </Link>
        ...
        {mobileMenuOpen && (
          ...
          >
            ...
            <Link
              href="#"
              className="mt-2 text-center px-4 py-2.5 rounded-lg text-sm font-semibold no-underline"
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-base" style={{ color: 'var(--text-secondary)' }}>
            Discover, compare, and monitor APIs across the web. Real-time uptime &amp; performance data.
          </p>
        </div>
          ...
          <div className="relative">
            ...
            {sortDropdownOpen && (
              <>
                ...
                <div
                  className="absolute right-0 top-full mt-1 z-50 rounded-lg border overflow-hidden py-1 min-w-[180px]"
                ...
                >
                  {SORT_OPTIONS.map((option) => (
                    <button
                      ...
                      style={{
                        background: sort === option.value ? 'var(--accent-glow)' : 'transparent',
                        color: sort === option.value ? 'var(--accent)' : 'var(--text-secondary)',
                      }}
                    >
                      {option.label}
                    </button>
        ...
        {/* Category tabs */}
        <div className="flex gap-2 overflow-x-auto pb-4 mb-6 scrollbar-none" style={{ scrollbarWidth: 'none' }}>
          {CATEGORIES.map((cat) => (
            <button
              key={cat}
        ...
        <div className="flex gap-6">
                    ...
                    >
                      <div
                        ...
                        onMouseEnter={(e) => {
                          ...
                          (e.currentTarget as HTMLElement).style.boxShadow = '0 0 20px rgba(59,130,246,0.1)';
                        }}
                        onMouseLeave={(e) => {
                      ...
                      >
                        ...
                        <button
                          ...
                          style={{
                            background: isWatched ? 'var(--accent-glow)' : 'var(--bg-secondary)',
                            color: isWatched ? 'var(--accent)' : 'var(--text-secondary)',
          ...
          {/* Right Sidebar — Top 5 Most Reliable */}
              ...
              <div className="flex flex-col gap-3">
                {top5Reliable.map((api, idx) => (
                  <Link
                    key={api.id}
                    href={`/marketplace/${api.slug}`}
                    className="flex items-center gap-3 p-2.5 rounded-lg transition-all no-underline group"
                  ...
                  >
                    ...
                    <span
                      ...
                      style={{
                        background: idx === 0 ? '#FFD700' : idx === 1 ? '#C0C0C0' : idx === 2 ? '#CD7F32' : 'var(--border)',
                        color: idx < 3 ? '#0A0F1E' : 'var(--text-secondary)',
                      }}
                    >
                      {idx + 1}
                    </span>
