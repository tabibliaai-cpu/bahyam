...
const sponsorTiers = [
  {
    ...
    icon: Star,
    color: '#F59E0B',
    description: 'Get your API in front of thousands of developers.',
    features: [
      'Logo on Live Feed page for 1 month',
      'Up to 10,000 impressions/month',
      'Click-through tracking dashboard',
      'Brand mention in weekly newsletter',
  ...
  },
  {
    ...
    icon: Zap,
    color: '#3B82F6',
    description: 'Maximum visibility with dedicated placement.',
    features: [
      'Everything in Featured, plus:',
      'Top banner placement on marketplace',
  ...
  },
  {
    ...
    icon: Target,
    color: '#8B5CF6',
    description: 'Full platform integration for maximum impact.',
    features: [
      'Everything in Premium, plus:',
      'Exclusive homepage banner',
      'Unlimited impressions',
      'Custom landing page for your API',
      'Direct integration in monitoring alerts',
...
const faqItems = [
  {
    q: 'Who sees my sponsored listing?',
    a: 'Your API will be shown to the 10,000+ developers who use PulseAPI daily to monitor their APIs. Our audience includes backend engineers, DevOps teams, and API product owners.',
  },
  {
    q: 'How is pricing determined?',
    a: 'Pricing is based on placement tier and expected impressions. All plans include real-time analytics. Custom packages are available for Enterprise clients with specific needs.',
  },
  {
    q: 'Can I pay with Bitcoin?',
    a: 'Absolutely! We support Bitcoin and Lightning Network payments via OpenNode for all sponsorship tiers. This makes it easy for international sponsors to pay without currency conversion fees.',
  },
  {
    q: 'How do I track performance?',
    a: 'Every sponsor gets a real-time dashboard showing impressions, click-throughs, and engagement metrics. Premium and Enterprise plans include demographic data and conversion tracking.',
  },
];
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--bg-primary)' }}>
      ...
      <section className="pt-28 pb-16 px-4 text-center relative overflow-hidden">
        <div className="absolute inset-0 opacity-30" style={{ background: 'radial-gradient(ellipse at 50% 0%, rgba(245,158,11,0.15) 0%, transparent 70%)' }} />
        <div className="max-w-3xl mx-auto relative z-10">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-sm mb-8 border" style={{ borderColor: 'var(--border)', color: 'var(--text-secondary)', background: 'var(--bg-secondary)' }}>
            <Megaphone className="w-4 h-4" style={{ color: 'var(--warning)' }} /> Developer-first sponsorship
          </div>
          <h1 className="text-4xl sm:text-5xl font-extrabold mb-6 leading-tight">
            Reach <span className="bg-clip-text text-transparent" style={{ backgroundImage: 'linear-gradient(135deg, #F59E0B, #EF4444)' }}>10,000+</span> developers watching APIs live
          </h1>
          <p className="text-lg mb-8" style={{ color: 'var(--text-secondary)' }}>
            PulseAPI is the #1 real-time API monitoring platform. Put your product in front of the developers who matter most.
          </p>
        </div>
      </section>
      ...
      <section className="px-4 pb-16">
        <div className="max-w-4xl mx-auto grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { icon: Activity, label: 'APIs Monitored', value: `${stats.totalApis || 20}+`, color: 'var(--accent)' },
            { icon: BarChart3, label: 'Daily Checks', value: `${(stats.totalChecks || 15000).toLocaleString()}+`, color: 'var(--success)' },
            { icon: Eye, label: 'Monthly Visitors', value: '10,000+', color: 'var(--warning)' },
            { icon: Target, label: 'Avg CTR', value: '3.2%', color: '#8B5CF6' },
          ].map((stat) => (
            <div key={stat.label} className="rounded-xl border p-4 text-center" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {sponsorTiers.map((tier) => {
              ...
              return (
                ...
                >
                  {tier.popular && (
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 rounded-full text-xs font-bold text-white" style={{ background: tier.color }}>
                      Most Popular
                    </div>
                  )}
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ background: `${tier.color}20` }}>
                  ...
                  <button
                    className={`w-full py-2.5 rounded-lg text-sm font-medium transition cursor-pointer ${tier.popular ? 'text-white hover:opacity-90' : 'border hover:bg-white/5'}`}
                    style={tier.popular ? { background: tier.color } : { borderColor: 'var(--border)' }}
                  >
                    Apply Now
                  </button>
                </div>
        ...
        <div className="max-w-xl mx-auto">
          <div className="rounded-xl border p-6 sm:p-8" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
            ...
            <p className="text-sm mb-6" style={{ color: 'var(--text-secondary)' }}>
              Tell us about your company and goals. We&apos;ll get back to you within 24 hours.
            </p>

            {submitted ? (
              <div className="text-center py-8">
                <Check className="w-12 h-12 mx-auto mb-3" style={{ color: 'var(--success)' }} />
                <h3 className="font-bold mb-1">Application Sent!</h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button
                  type="submit"
                  className="w-full py-3 rounded-lg text-white font-medium text-sm transition hover:opacity-90 flex items-center justify-center gap-2 cursor-pointer"
